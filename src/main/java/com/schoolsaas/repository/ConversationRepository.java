...
public interface ConversationRepository extends JpaRepository<Conversation, UUID> {
    ...
    Page<Conversation> findBySchoolIdOrderByUpdatedAtDesc(UUID schoolId, Pageable pageable);
    ...
    @Query("SELECT c FROM Conversation c JOIN ConversationParticipant cp ON c.id = cp.conversationId WHERE cp.userId = :userId AND c.schoolId = :schoolId ORDER BY c.updatedAt DESC")
    Page<Conversation> findByParticipantUserIdAndSchoolId(UUID userId, UUID schoolId, Pageable pageable);
