...
CREATE INDEX IF NOT EXISTS idx_content_scheduled ON content_items(scheduled_publish_at) WHERE scheduled_publish_at IS NOT NULL AND status = 'SCHEDULED';

-- Create index for featured content
DROP INDEX IF EXISTS idx_content_featured;
CREATE INDEX IF NOT EXISTS idx_content_featured ON content_items(school_id, is_featured) WHERE is_featured = TRUE;
